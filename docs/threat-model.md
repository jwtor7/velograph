...
| Private files enter a container layer or CI context | `.dockerignore`, no source-export/auth mounts, image-layer audit, SBOM-enabled multi-architecture CI                         | A maintainer can still use an unsafe local override; do not publish images built from an untrusted context |
...
| Dependency or build compromise                      | Frozen lockfile, pinned Actions, clean Node 20/26 installs, multi-architecture build, SBOM, manual dependency/licence review | Base images and action pins require routine review and digest updates before a release                     |
...
disabled. The image build context excludes local data, exports, credentials,
logs, fixtures, and Git metadata. No image is an acceptable backup of user
data.
...
1. clean frozen-lockfile verification on Node 20 and Node 26;
...
4. multi-architecture (`linux/amd64`, `linux/arm64`) build with SBOM
   attestation enabled;
5. a scan of the exact image or artifact intended for publication; and
6. independent review of the SBOM, dependencies, licence status, and this
   threat model.
