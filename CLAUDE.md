...
**Current state:** pre-code. The repo contains only the PRD; no build system exists yet. When scaffolding, use the PRD §11.2 stack: TypeScript monorepo, pnpm, React client, Node.js loopback API, SQLite with ordered migrations, streaming CSV parser, pure analytics package. Replace this paragraph with real build/test/lint commands as soon as they exist.
