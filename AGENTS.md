...
If you start a server during a session, you own it. The maintainer must never have to
...
- Stop what you started before ending the turn, unless the maintainer is actively using the
  app — in which case say so and tell them `pnpm app:stop`.
- After changing `apps/api` or `apps/web`, use `pnpm app:restart`. A stale API serving a
  freshly built web client produces a UI calling endpoints that do not exist.
- Never import synthetic fixtures into the default data directory; it may hold real rides.
  Point fixture imports at an explicit throwaway `VELO_DATA_DIR`.
