...
import { extname, join as join4, normalize } from "node:path";
...
// packages/importers/src/inventory.ts
function inventoryFiles(files) {
  const seen = /* @__PURE__ */ new Set();
  return files.map((f) => {
...
function movingTime(route2, thresholdMs) {
  let ms = 0;
  let sawTiming = false;
  for (const seg of route2) {
    for (let i = 0; i < seg.points.length - 1; i++) {
...
function handle(req, res, opts, now, server) {
...
function route(req, res, opts, url, method, now) {
  const db = opts.db;
  const path = url.pathname;
  ...
  if (method === "PUT" && path === "/api/settings") {
    readJsonBody(req, 1024 * 1024).then((body) => {
      ...
      saveSettings(db, { ...loadSettings(db), ...s });
      send(res, 200, { settings: loadSettings(db) });
    }).catch(() => send(res, 400, { error: "invalid_body" }));
    return;
  }
  if (method === "POST" && path === "/api/import/inventory") {
    readJsonBody(req, MAX_IMPORT_BODY_BYTES).then((body) => {
  ...
  if (method === "POST" && path === "/api/backup") {
    readJsonBody(req, MAX_PATH_BODY_BYTES).then(async (body) => {
...
function serveStatic(res, staticDir, path) {
  ...
  let file = join4(staticDir, rel);
  if (!file.startsWith(normalize(staticDir))) {
    res.writeHead(403, { "Content-Type": "text/plain" });
    res.end("forbidden");
    return;
  }
  if (!existsSync3(file) || statSync(file).isDirectory()) {
    file = join4(staticDir, "index.html");
    if (!existsSync3(file)) {
...
async function main() {
  ...
  const webDist = join5(dirname3(fileURLToPath2(import.meta.url)), "..", "..", "web", "dist");
  const server = createApiServer({
    db,
    dbPath,
    ...existsSync4(webDist) ? { staticDir: webDist } : {}
  });
